...
class BeneficiaryProfile(models.Model):
    # Link to the user account who owns this beneficiary
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='beneficiaries')

    full_name = models.CharField(max_length=100, blank=True)
    mobile = models.CharField(max_length=15, validators=[mobile_validator], blank=True, null=True)
    email = models.EmailField(
        max_length=100,
        validators=[EmailValidator(message="Enter a valid email address")],
        blank=True,
    ...
    relation = models.CharField(max_length=50, blank=True, help_text="Relationship with account holder")
    image = models.ImageField(upload_to='beneficiary_images/', null=True, blank=True)
    ...
    def save(self, *args, **kwargs):
        if not self.full_name and self.user:
            self.full_name = "Beneficiary"
        super().save(*args, **kwargs)

    def __str__(self):
        return self.full_name or f"Beneficiary of {self.user.username}"
